...
public sealed class MongoDatabaseProvider : IDatabaseProvider, IDBQuery
{
    ...
    private static FilterDefinition<T> BuildSearchFilter<T>(
        FilterDefinitionBuilder<T> builder,
        SearchFilter searchFilter
    )
    {
        return searchFilter.Operator switch
        {
            SearchOperator.EQ =>
                builder.Eq(
                    searchFilter.Field,
            ...
            SearchOperator.Contains =>
                builder.Regex(
                    ...
                    new BsonRegularExpression(
                        Regex.Escape(
                            Convert.ToString(searchFilter.Value) ?? ""
                        ),
                        "i"
                    )
                ),
            ...
            _ => throw new NotSupportedException(
                $"Search operator '{searchFilter.Operator}' is not supported by MongoDB."
            )
        };

    }
    private static IEnumerable<object?> ToValues(object? value)
    {
        if (value is null)
        {
            return [];
    ...
    where T : DBModel
    {
        ...
        foreach (var searchFilter in searchParam.Filters)
        {
            var filter = BuildSearchFilter<T>(
                builder,
                searchFilter
            );
        ...
        return searchParam.Condition == SearchCondition.Or
            ? builder.Or(filters)
            : builder.And(filters);
    }
    ...
    public string GenerateDebugQuery<T>(
      SearchParam searchParam, int skip, int limit
  )
      where T : DBModel
    {
        ArgumentNullException.ThrowIfNull(searchParam);

        var filterDefinition = BuildFilter<T>(
            searchParam
        );
        ...
        var renderedFilter = filterDefinition.Render(
            new RenderArgs<T>(
                documentSerializer,
                serializerRegistry
            )
