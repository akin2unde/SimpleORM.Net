...
public sealed class MongoDatabaseProvider:IDatabaseProvider,IDBQuery
{
    ...
    public Task<T?> GetByCode<T>(string code,CancellationToken ct=default) where T:DBModel
    {
        ...
        {
            Field=nameof(DBModel.Code),Operator=SearchOperator.Equal,Value=code
        }
        );
    ...
    {
        collection=_m.GetMetadata<T>().TableName,skip,limit,note="MongoDB filter is generated by provider at execution time."
    }
    ...
    /// <inheritdoc />
    public Task<T?> QuerySingle<T>(string q,object? p=null,CancellationToken ct=default)=>throw new NotSupportedException("Use dynamic raw Mongo query in MVP or IDataService<T>.");


    /// <inheritdoc />
    public Task<IReadOnlyList<T>> Query<T>(string q,object? p=null,CancellationToken ct=default)=>throw new NotSupportedException("Use dynamic raw Mongo query in MVP or IDataService<T>.");
    ...
    private FilterDefinition<T> Filter<T>(SearchParam p) where T:DBModel
    {
        ...
        foreach(var x in p.Filters)
        {
            ...
            {
                SearchOperator.Equal=>b.Eq(c.ColumnName,x.Value),SearchOperator.NotEqual=>b.Ne(c.ColumnName,x.Value),SearchOperator.Contains=>b.Regex(c.ColumnName,new BsonRegularExpression(System.Text.RegularExpressions.Regex.Escape(x.Value?.ToString()??""),"i")),SearchOperator.In=>b.In(c.ColumnName,((System.Collections.IEnumerable)x.Value!).Cast<object?>()),_=>throw new NotSupportedException($"Operator {x.Operator} not implemented in Mongo MVP.")
            }
            );
    ...
    private FilterDefinition<T> Scoped<T>(FilterDefinition<T> f,bool includeDeleted) where T:DBModel
    {
        ...
        if(_o.MultiTenancy.Enabled)all.Add(b.Eq(x=>x.TenantCode,_t.GetTenantCode()??throw new InvalidOperationException("Tenant required.")));
